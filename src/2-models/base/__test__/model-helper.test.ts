...
import { TestTenantScopedModel } from "./tenant-scoped-model.test";
...
class _Model extends TenantScopedModel<{ e: TestTenantScopedEntity }> {
  increment() {
    this.update(
      "e",
      this.dependencies.e.set({ seq: this.dependencies.e.seq + 1 })
    );
  }
}
...
export class TestNestedModel extends TenantScopedModel<D> implements D {
  ...
  addDeltaModel(init: Pick<TestTenantScopedEntity, "seq">) {
    ...
    this.update("deltaModels", [...this.deltaModels, model]);
    return model;
  }
}
